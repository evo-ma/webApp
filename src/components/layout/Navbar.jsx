...
export default function Navbar() {
  ...
  const [activeMenu, setActiveMenu] = useState(null) // 'produits' | 'solutions' | null
  ...
  return (
    <header
      ...
      className={cn(
        ...
        scrolled
          ? 'bg-dark/90 backdrop-blur-xl border-b border-white/[0.06] shadow-xl'
          : 'bg-transparent'
      )}
    >
      <nav className="max-w-7xl mx-auto px-5 md:px-10">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden lg:flex items-center gap-0.5">
            ...
            {/* PRODUITS dropdown */}
            <NavDropdownBtn
              label={t('nav.produits')}
              active={isActive('/produits') || activeMenu === 'produits'}
              open={activeMenu === 'produits'}
              onClick={() => toggle('produits')}
            />

            {/* SOLUTIONS dropdown */}
            <NavDropdownBtn
              label={t('nav.solutions')}
              active={isActive('/solutions') || activeMenu === 'solutions'}
        ...
        {activeMenu && (
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="max-w-7xl mx-auto px-10 py-8">
              ...
              {/* PRODUITS mega menu — Téléphonie (wide) + IA (narrow) */}
              {activeMenu === 'produits' && (
                <div className="flex gap-0">
                  ...
                  <div className="flex-[3] pr-8">
                    ...
                    <div className="flex items-center gap-3 mb-4">
                      <span className="text-[10px] font-bold text-white/30 tracking-[0.18em] uppercase">
                        {t('nav.telephony')}
                      </span>
                    ...
                    <div className="grid grid-cols-2 gap-0.5">
                      {TELEPHONY.map(({ key, path, icon }) => {
                        ...
                        return (
                          <MegaItem key={key} to={path} Icon={Icon}
                            title={t(`solutions.${key}.name`)}
                            desc={t(`solutions.${key}.short`)}
                            color="navy" />
                        )
                      })}
                    </div>
                  ...
                  {/* Vertical divider */}
                  <div className="w-px bg-white/[0.06] self-stretch mx-0 flex-shrink-0" />
              ...
              {/* SOLUTIONS mega menu — ECHO + Bayling */}
                    ...
                    return (
                      <Link key={key} to={path}
                        className={cn(
                          ...
                          coming
                            ? 'border-white/[0.06] bg-white/[0.02] opacity-60 cursor-default pointer-events-none'
                        ...
                        <div className={cn(
                          ...
                          coming
                            ...
                            : 'bg-orange/10 border-orange/25 text-orange group-hover:bg-orange/15'
                        )}>
                          {Icon && <Icon size={20} />}
                        </div>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            {coming && (
                              <span className="text-[9px] font-bold tracking-widest uppercase bg-white/[0.07] text-white/40 border border-white/10 rounded-full px-2 py-0.5">
                                {t(`evo_solutions.${key}.badge`)}
                              </span>
                            )}
                            {!coming && (
                              <span className="text-[9px] font-bold tracking-widest uppercase bg-orange/15 text-orange border border-orange/25 rounded-full px-2 py-0.5">
                                {t(`evo_solutions.${key}.badge`)}
                              </span>
                            )}
                          </div>
                          <p className="text-xs text-white/40 leading-relaxed">
                            {t(`evo_solutions.${key}.short`)}
                          </p>
                          {!coming && (
            ...
            <div className="px-5 py-6 space-y-1">
                  ...
                  {TELEPHONY.map(({ key, path, icon }) => {
                    ...
                    return (
                      <Link key={key} to={path} className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-white/5 transition-colors group">
                        <div className="w-7 h-7 flex items-center justify-center rounded-lg bg-navy/30 text-orange/70 group-hover:text-orange flex-shrink-0">
                          {Icon && <Icon size={13} />}
                        </div>
                  ...
                  {AI_PRODUCTS.map(({ key, path, icon }) => {
                    ...
                    return (
                      <Link key={key} to={path} className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-orange/5 transition-colors group">
                        <div className="w-7 h-7 flex items-center justify-center rounded-lg bg-orange/5 border border-orange/15 text-orange/70 group-hover:text-orange flex-shrink-0">
                          {Icon && <Icon size={13} />}
                        </div>
              ...
              <MobileSection title={t('nav.solutions')}>
                <div className="pl-1 pt-1 space-y-1">
                  {SOLUTIONS_EVO.map(({ key, path, coming }) => (
                    <Link key={key} to={path}
                      className={cn('flex items-center gap-3 px-3 py-2 rounded-lg transition-colors', coming ? 'opacity-50 pointer-events-none' : 'hover:bg-orange/5')}>
              ...
              <div className="pt-2 border-t border-white/[0.06] space-y-1">
                {[
                  ...
                  <Link key={to} to={to} className="block px-3 py-2.5 text-sm text-white/60 hover:text-white rounded-lg hover:bg-white/5 transition-colors">
                    {label}
                  </Link>
                ))}
              </div>
  ...
  return (
    <button onClick={onClick}
      className={cn(
        ...
        active ? 'text-white bg-white/5' : 'text-white/55 hover:text-white hover:bg-white/5'
      )}>
      {label}
  ...
  return (
    <Link to={to}
      className={cn(
        ...
        color === 'orange'
          ? 'hover:bg-orange/[0.04] hover:border-orange/15'
          : 'hover:bg-white/[0.04] hover:border-white/[0.08]'
      )}>
      <div className={cn(
        'w-8 h-8 flex items-center justify-center rounded-lg border flex-shrink-0 transition-all duration-200',
        color === 'orange'
  ...
  return (
    <div>
      <button onClick={() => setOpen(!open)}
        className="w-full flex items-center justify-between px-3 py-2.5 text-sm font-semibold text-white/80 hover:text-white rounded-lg hover:bg-white/5 transition-colors">
        {title}
        <ChevronDown size={14} className={cn('transition-transform duration-200', open && 'rotate-180')} />
      </button>
      <AnimatePresence>
